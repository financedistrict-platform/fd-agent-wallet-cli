...
- `fdx wallet <method>` — Wallet MCP tools (transfers, swaps, yield, X402 payments)
- `fdx prism <method>` — Prism MCP tools (payments, settlements, accounts)
...
- Updated all documentation examples to use new `fdx wallet <method>` and `fdx prism <method>` syntax

### Deprecated

- `fdx call <method>` — use `fdx wallet <method>` or `fdx prism <method>` instead (exits with error)
...
fdx wallet getTokenPrice --token ETH
fdx prism listPayments               # new: Prism tools
...
**AI agent operators:** Update tool invocation configs from `fdx call <method>` to `fdx wallet <method>`. Prism tools now available via `fdx prism <method>`.
